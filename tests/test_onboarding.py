...
class OnboardingTests(unittest.TestCase):
    def test_save_project_config_writes_model_and_character_settings(self):
        ...
        with tempfile.TemporaryDirectory() as d:
            ...
            save_project_config(
                config_path,
                image_api_key_env='AIDP_IMAGE_API_KEY',
            ...
            self.assertEqual(data['image']['api_key_env'], 'AIDP_IMAGE_API_KEY')
            self.assertEqual(data['image']['model'], 'gpt-image-2')
            self.assertEqual(data['image']['base_url'], 'https://example.test/v1')
            self.assertEqual(data['journey']['first_stop_id'], 'british-museum')
